...
| **Command/Tool**             | **Description**                                                                                           | **Attacker’s Perspective**                                                                                     |
|------------------------------|-----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| **`tail`**                   | Displays the last few lines of a log file in real-time.                                                   | - Monitor live logs for security events triggered by attacks.<br>- Detect defensive tools in use.           |
| | **1. `tail -f /var/log/syslog`**: Continuously views the system log file.                                                              | Observe live system activities or responses to malicious actions.                                            |
| **`grep`**                   | Searches log files for specific patterns or keywords.                                                     | - Filter logs for sensitive information like usernames or error codes.<br>- Search for specific attack patterns. |
| | **1. `grep "Failed password" /var/log/auth.log`**: Searches for failed login attempts in authentication logs.                         | Identify weak accounts or brute-force attack outcomes.                                                      |
| **`journalctl`**             | Views logs generated by systemd.                                                                          | - Analyze logs for security events like service restarts or crashes.<br>- Identify actions taken by administrators. |
| | **1. `journalctl -u sshd`**: Views logs related to the SSH service.                                                                    | Detect SSH misconfigurations or security policies.                                                          |
| **`logrotate`**              | Manages log rotation to prevent log files from consuming excessive disk space.                            | - Exploit misconfigured log rotation that causes overwriting or data loss.<br>- Trigger excessive logging to exhaust disk space. |
| | **1. `cat /etc/logrotate.conf`**: Displays the logrotate configuration.                                                                | Identify gaps in log retention policies.                                                                    |
| **`cat`**                    | Views log file contents.                                                                                  | - Analyze log entries to find evidence of misconfigurations or sensitive data.                              |
| | **1. `cat /var/log/kern.log`**: Displays kernel-related logs.                                                                          | Inspect kernel activity for potential vulnerabilities or denial-of-service issues.                          |
| **`awk`**                    | Parses and extracts specific fields from log files.                                                       | - Extract relevant data from logs for analysis, such as IP addresses or timestamps.                         |
| | **1. `awk '{print $1, $2, $3, $NF}' /var/log/syslog`**: Displays timestamp and the last field from syslog entries.                    | Narrow down log entries to relevant details for further analysis.                                           |
| **`sed`**                    | Edits or filters log file content.                                                                        | - Remove traces of attacker activity from logs.<br>- Mask specific data in logs to evade detection.         |
| **`rkhunter`**               | Scans for rootkits, backdoors, and known vulnerabilities.                                                  | - Detect rootkits or tamper with logs to avoid detection.                                                   |
| **`chkrootkit`**             | Similar to rkhunter, checks for signs of rootkits.                                                        | - Analyze detection mechanisms to evade system monitoring.                                                  |
| **Application-Specific Logs**|                                                                                                           |                                                                                                              |
| | **1. `cat /var/log/apache2/access.log`**: Views Apache web server access logs.                                                         | Identify vulnerable endpoints or active users for exploitation.                                             |
| | **2. `cat /var/log/mysql/error.log`**: Views MySQL database error logs.                                                                | Search for misconfigurations or queries that reveal database vulnerabilities.                                |
| **`auditd`**                 | Monitors system calls and tracks access to sensitive files.                                                | - Identify sensitive operations or files being monitored.<br>- Evade audit policies to perform stealthy actions. |
| | **1. `auditctl -l`**: Lists active audit rules.                                                                                       | Determine which actions are being logged for evasion or testing.                                            |
| **Security Log Analysis Tools**|                                                                                                        |                                                                                                              |
| | **1. `fail2ban-client status`**: Views blocked IPs and status of fail2ban.                                                             | Determine if attacker activities have been detected and blocked.                                            |
| | **2. `ufw status verbose`**: Views UFW (firewall) logs.                                                                                | Analyze firewall activity for gaps or open ports.                                                           |
